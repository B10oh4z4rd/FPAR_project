...
#       self.mmapPredictor.add_module('print1',Print())
    ...
    def forward(self, inputVariable):
        ...
        for t in range(inputVariable.size(0)):
            ...
            expanded = attentionMAP.expand_as(feature_conv)
            nbncopy = feature_convNBN.clone()
            attentionFeat = nbncopy*expanded
            if t == 0:
              map_predictions = self.mmapPredictor(feature_convNBN)
            else:
              prediction = self.mmapPredictor(feature_convNBN)
